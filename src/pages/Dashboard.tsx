  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 p-8 text-white shadow-lg">
        <h1 className="mb-2 text-3xl font-bold md:text-4xl">Welcome to Network+ Study Platform</h1>
        <p className="text-lg opacity-90">
          Independent study resource for Network+ exam preparation
        </p>
      </div>

      {/* Overall Progress */}
      <div className="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
        <h2 className="mb-4 text-2xl font-bold text-gray-900 dark:text-white">Your Progress</h2>
          ...
          {categories.map((category) => {
            ...
            return (
              <Link
                key={category.id}
                to={category.path}
                className="transform rounded-lg bg-white p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:bg-gray-800"
              >
                <div className="mb-4 flex items-center justify-between">
                  <span className="text-4xl" role="img" aria-label={category.name}>
                    {category.icon}
                  </span>
                  {progress.componentsCompleted === progress.totalComponents &&
                    progress.totalComponents > 0 && (
                      <svg
