...
export const TroubleshootingScenarios: React.FC<TroubleshootingScenariosProps> = ({ onProgressUpdate }) => {
  ...
  const calculateScore = useCallback((scenario: TroubleshootingScenario, response: ScenarioResponse): number => {
    let score = 0;
  ...
  const submitResponse = useCallback(() => {
    ...
    const correctCount = Array.from(responses.values()).filter(
      r => TROUBLESHOOTING_SCENARIOS.find(s => s.id === r.scenarioId)?.correctLayer === r.selectedLayer
  ...
  }, [currentResponse, currentScenario, currentScenarioIndex, filteredScenarios.length, calculateScore, responses, onProgressUpdate]);
  ...
  return (
    <div className="troubleshooting-scenarios" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      <div className="header" style={{ marginBottom: '30px' }}>
        ...
        <div className="stats" style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{stats.attempted}/{TROUBLESHOOTING_SCENARIOS.length}</div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{Math.round(stats.avgScore)}%</div>
        ...
        <div className="filters" style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <div>
            <label style={{ marginRight: '8px', fontWeight: 'bold' }}>Difficulty:</label>
            <select
      ...
      <div className="scenario-card" style={{
        ...
        <div className="scenario-header" style={{ marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              <span style={{
                padding: '4px 12px',
                backgroundColor: currentScenario.difficulty === 'easy' ? '#4CAF50' : currentScenario.difficulty === 'medium' ? '#FF9800' : '#f44336',
                color: 'white',
                borderRadius: '12px',
                fontSize: '12px',
                fontWeight: 'bold'
        ...
        <div className="response-section" style={{ marginTop: '30px' }}>
          ...
          <div className="layer-selection" style={{ marginBottom: '25px' }}>
            ...
            <div style={{
              ...
              {[7, 6, 5, 4, 3, 2, 1].map(layer => (
                <button
                  key={layer}
                  onClick={() => setCurrentResponse(prev => ({ ...prev, selectedLayer: layer as OSILayerNumber }))}
                  style={{
                    padding: '15px',
                    backgroundColor: currentResponse.selectedLayer === layer ? LAYER_COLORS[layer] : '#f5f5f5',
                ...
                >
                  ...
                  <div style={{ fontSize: '12px', marginTop: '5px' }}>{LAYER_NAMES[layer as OSILayerNumber]}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="explanation-section" style={{ marginBottom: '25px' }}>
            ...
            <textarea
              ...
              onChange={(e) => setCurrentResponse(prev => ({ ...prev, explanation: e.target.value }))}
            ...
            <div style={{ marginTop: '5px', fontSize: '12px', color: '#666' }}>
              Word count: {currentResponse.explanation?.split(' ').filter(w => w.length > 0).length || 0} / 100
              {(currentResponse.explanation?.split(' ').filter(w => w.length > 0).length || 0) >= 100 && (
          ...
          <div className="solution-section" style={{ marginBottom: '25px' }}>
            ...
            <textarea
              ...
              onChange={(e) => setCurrentResponse(prev => ({ ...prev, solution: e.target.value }))}
            ...
            />
            <div style={{ marginTop: '5px', fontSize: '12px', color: '#666' }}>
              Word count: {currentResponse.solution?.split(' ').filter(w => w.length > 0).length || 0} / 50
          ...
          {/* Hints Section */}
          <div className="hints-section" style={{
            marginBottom: '25px',
            padding: '15px',
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h4 style={{ margin: 0 }}>Hints Available: {usedHints.has(currentScenario.id) ? 'Used' : 'Not Used'}</h4>
          ...
          <div style={{ display: 'flex', gap: '15px', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', gap: '15px' }}>
              ...
              <button
                ...
                style={{
                  padding: '12px 24px',
                  backgroundColor: currentScenarioIndex >= filteredScenarios.length - 1 ? '#ccc' : '#2196F3',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
                  cursor: currentScenarioIndex >= filteredScenarios.length - 1 ? 'not-allowed' : 'pointer',
                  fontWeight: 'bold'
                }}
              >
                Next →
        ...
        {responses.has(currentScenario.id) && (
          <div style={{
            marginTop: '30px',
            padding: '20px',
            backgroundColor: responses.get(currentScenario.id)?.selectedLayer === currentScenario.correctLayer ? '#e8f5e9' : '#ffebee',
            borderRadius: '8px',
            border: `2px solid ${responses.get(currentScenario.id)?.selectedLayer === currentScenario.correctLayer ? '#4CAF50' : '#f44336'}`
          }}>
            <h4 style={{ marginTop: 0 }}>
              {responses.get(currentScenario.id)?.selectedLayer === currentScenario.correctLayer ? '✓ Correct!' : '✗ Incorrect'}
            </h4>
            <p>
              <strong>Correct Layer:</strong> Layer {currentScenario.correctLayer} ({LAYER_NAMES[currentScenario.correctLayer]})
            </p>
            <p>
      ...
      <div style={{
        ...
        <h4 style={{ marginTop: 0 }}>Scenario Progress ({currentScenarioIndex + 1} of {filteredScenarios.length})</h4>
        <div style={{
          display: 'grid',
          ...
          {filteredScenarios.map((scenario, index) => {
            ...
            return (
              <button
                ...
                style={{
                  padding: '10px',
                  backgroundColor: response ? (isCorrect ? '#4CAF50' : '#f44336') : (index === currentScenarioIndex ? '#2196F3' : '#f5f5f5'),
