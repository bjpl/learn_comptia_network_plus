/**
 * Packet exchange visualization component
 */

import React from 'react';
import type { ScanResult } from '../types';

interface PacketExchangeProps {
  result: ScanResult;
}

export const PacketExchange: React.FC<PacketExchangeProps> = ({ result }) => {
  return (
    <div
    ...
    >
      ...
      <div className="exchange-summary">
        <h5>
          Scan Result: <span className={`state-${result.state}`}>{result.state.toUpperCase()}</span>
        </h5>
        {result.banner && (
