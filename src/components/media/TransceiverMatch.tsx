...
const TRANSCEIVER_SPECS: TransceiverSpec[] = [
  { formFactor: 'GBIC', fullName: 'Gigabit Interface Converter', speed: '1G', channels: 1, size: 'Large', introduced: 1995, useCase: 'Legacy gigabit connections', keyFeature: 'First hot-swappable transceiver' },
  { formFactor: 'SFP', fullName: 'Small Form-Factor Pluggable', speed: '1G', channels: 1, size: 'Small (Mini-GBIC)', introduced: 2001, useCase: 'Standard gigabit, copper or fiber', keyFeature: 'Replaced GBIC, hot-swappable' },
  { formFactor: 'SFP+', fullName: 'Enhanced Small Form-Factor Pluggable', speed: '10G', channels: 1, size: 'Same as SFP', introduced: 2006, useCase: '10G datacenter, servers, storage', keyFeature: 'Same size as SFP but 10x speed' },
  { formFactor: 'SFP28', fullName: 'SFP 28Gbps', speed: '25G', channels: 1, size: 'Same as SFP', introduced: 2014, useCase: '25G datacenter, cloud infrastructure', keyFeature: '25G single lane' },
  { formFactor: 'QSFP', fullName: 'Quad Small Form-Factor Pluggable', speed: '4G', channels: 4, size: 'Slightly larger', introduced: 2006, useCase: '4x1G aggregation', keyFeature: '4 channels in one module' },
  { formFactor: 'QSFP+', fullName: 'Quad SFP Plus', speed: '40G', channels: 4, size: 'Same as QSFP', introduced: 2009, useCase: '40G spine-leaf datacenter', keyFeature: '4x10G = 40G total' },
  { formFactor: 'QSFP28', fullName: 'Quad SFP 28Gbps', speed: '100G', channels: 4, size: 'Same as QSFP', introduced: 2014, useCase: '100G modern datacenter, hyperscale', keyFeature: '4x25G = 100G total' },
  { formFactor: 'QSFP-DD', fullName: 'Quad SFP Double Density', speed: '400G', channels: 8, size: 'Double density', introduced: 2017, useCase: '400G cloud, AI/ML workloads', keyFeature: '8x50G = 400G total' },
  { formFactor: 'CFP', fullName: 'C Form-Factor Pluggable', speed: '100G+', channels: 1, size: 'Very large', introduced: 2009, useCase: '100G+ WAN, metro networks', keyFeature: 'High power, long reach' }
];
...
const TROUBLESHOOTING_TIPS: TroubleshootingTip[] = [
  {
    ...
    causes: ['Dirty fiber connector', 'Wrong transceiver type', 'Incompatible wavelength', 'Fiber not connected', 'Damaged cable'],
    solutions: ['Clean fiber with approved cleaner', 'Verify transceiver compatibility', 'Check wavelength matches (850nm vs 1310nm)', 'Reseat connections', 'Test with different cable']
  },
  {
    symptom: 'Intermittent connectivity / Packet loss',
    causes: ['Low optical power (dBm)', 'EMI interference', 'Loose connection', 'Temperature issues', 'Fiber bend radius exceeded'],
    solutions: ['Check dBm levels (should be -3 to -20 dBm)', 'Shield cables properly', 'Secure all connections', 'Improve ventilation', 'Ensure proper cable routing']
  },
  {
    symptom: 'Incompatible transceiver error',
    causes: ['Non-approved vendor module', 'Wrong form factor', 'Speed mismatch', 'Firmware incompatibility'],
    solutions: ['Use vendor-approved transceivers', 'Verify SFP vs SFP+ vs QSFP', 'Match speed requirements', 'Update switch firmware']
  },
  {
    symptom: 'Distance limitation errors',
    causes: ['Wrong transceiver type (-SR vs -LR)', 'Multimode vs single-mode mismatch', 'Power budget exceeded', 'Wrong fiber grade (OM3 vs OM4)'],
    solutions: ['Use -LR for long distances', 'Match fiber type to transceiver', 'Calculate power budget', 'Upgrade to OM4 for 10G']
  }
];
            ...
            <CardContent>
              <div className="space-y-4">
                {TRANSCEIVER_SPECS.map((spec) => (
                  <div key={spec.formFactor} className="p-4 border rounded-lg hover:bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <Badge className="text-lg px-3 py-1">{spec.formFactor}</Badge>
                          <span className="text-sm text-gray-500">({spec.introduced})</span>
                        </div>
                        <div className="text-sm font-medium mb-1">{spec.fullName}</div>
                        <div className="text-sm text-gray-600">
                          <strong>Speed:</strong> {spec.speed} | <strong>Channels:</strong> {spec.channels} | <strong>Size:</strong> {spec.size}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex items-start gap-2 text-sm">
                          ...
                          <span className="text-gray-600">{spec.keyFeature}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    <strong className="text-blue-900">Exam Tip:</strong> Remember the pattern - QSFP = 4x speed of equivalent SFP (QSFP+ is 4x10G = 40G, QSFP28 is 4x25G = 100G). SFP+ modules can fit in SFP ports but will run at reduced 1G speed.
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <TabsContent value="bidi-wdm" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-6">
              {WDM_TYPES.map((wdm) => (
                <div key={wdm.name} className="p-4 border-2 rounded-lg">
                  <div className="flex items-center gap-2 mb-3">
                    <Badge variant="outline" className="text-lg px-3 py-1">{wdm.name}</Badge>
                    <span className="text-sm font-medium text-gray-700">{wdm.fullName}</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <div className="font-semibold text-gray-700 mb-1">Technical Specs</div>
                      <div className="space-y-1 text-gray-600">
                        <div><strong>Channels:</strong> {wdm.channels}</div>
                        <div><strong>Wavelength Range:</strong> {wdm.wavelengthRange}</div>
                        <div><strong>Channel Spacing:</strong> {wdm.spacing}</div>
                      </div>
                    </div>
                    <div>
                      <div className="font-semibold text-gray-700 mb-1">Applications</div>
                      <div className="space-y-1 text-gray-600">
                        <div><strong>Primary Use:</strong> {wdm.useCase}</div>
                        <div><strong>Capacity Benefit:</strong> {wdm.capacity}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    <strong className="text-green-900">BiDi Explained:</strong> Bidirectional transceivers use different wavelengths for transmit (Tx) and receive (Rx) on a SINGLE fiber strand. One end transmits at 1270nm / receives at 1330nm, while the other end does the opposite. This cuts fiber usage in half and reduces installation costs.
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    <strong className="text-purple-900">Cost Savings:</strong> WDM technologies multiply fiber capacity without adding cables. DWDM can carry 80+ channels on a single fiber pair, replacing what would otherwise require 80 separate fiber pairs - massive cost savings for long-distance runs.
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-4">
              {TROUBLESHOOTING_TIPS.map((tip, index) => (
                <div key={index} className="p-4 border rounded-lg">
                  <div className="flex items-start gap-2 mb-3">
                    ...
                    <div className="font-semibold text-lg">{tip.symptom}</div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <div className="text-sm font-semibold text-red-700 mb-2">Possible Causes:</div>
                    ...
                    <div>
                      <div className="text-sm font-semibold text-green-700 mb-2">Solutions:</div>
                      <ul className="text-sm space-y-1">
              ...
              <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    <strong className="text-yellow-900">Optical Power Budget:</strong> Always check dBm levels. Typical ranges: -3 to -10 dBm (good), -10 to -20 dBm (acceptable), below -20 dBm (check for issues). Use an optical power meter for accurate measurements.
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <TabsContent value="exam-prep" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Critical knowledge points for exam success
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                <h3 className="font-semibold text-blue-900 mb-2">Key Exam Topics - LO 1.4</h3>
                <ul className="text-sm space-y-2">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span><strong>Form Factor Identification:</strong> Distinguish GBIC, SFP, SFP+, QSFP, QSFP+, QSFP28, CFP by size, speed, and use case</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span><strong>Speed Capabilities:</strong> SFP=1G, SFP+=10G, QSFP+=40G, QSFP28=100G</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span><strong>Distance Limitations:</strong> -SR (short range, multimode, 100-550m), -LR (long range, single-mode, 10km), -ER (extended range, 40km)</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span><strong>Fiber Compatibility:</strong> Multimode (850nm, orange cable) vs Single-mode (1310/1550nm, yellow cable)</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span><strong>BiDi Operation:</strong> Single fiber, different Tx/Rx wavelengths, cost savings</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span><strong>WDM Technologies:</strong> CWDM (20nm spacing, 8-18 channels), DWDM (0.4-0.8nm spacing, 40+ channels)</span>
                  </li>
                </ul>
              </div>

              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                <h3 className="font-semibold text-green-900 mb-2">Common Exam Scenarios</h3>
                <div className="text-sm space-y-3">
                  <div>
                    <strong className="text-green-800">Q: Connect two buildings 8km apart at 10Gbps?</strong>
                    <div className="mt-1 text-green-700">A: 10GBASE-LR (SFP+) with single-mode fiber - long range capability</div>
                  </div>
                  <div>
                    <strong className="text-green-800">Q: Rack-to-rack connection, 5m, 10Gbps, cost-effective?</strong>
                    <div className="mt-1 text-green-700">A: SFP+ DAC (Direct Attach Copper) - lowest cost for short distances</div>
                  </div>
                  <div>
                    <strong className="text-green-800">Q: SFP+ module in SFP port - what happens?</strong>
                    <div className="mt-1 text-green-700">A: Physical incompatibility - SFP+ will NOT fit in SFP-only port. However, SFP modules CAN work in SFP+ ports at reduced 1G speed</div>
                  </div>
                  <div>
                    <strong className="text-green-800">Q: 100G datacenter spine, 100m distance?</strong>
                    <div className="mt-1 text-green-700">A: QSFP28 100GBASE-SR4 with OM4 multimode fiber</div>
                  </div>
                </div>
              </div>

              <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                <h3 className="font-semibold text-purple-900 mb-2">Memory Aids</h3>
                <ul className="text-sm space-y-2">
                  <li className="flex items-start gap-2">
                    <Lightbulb className="h-4 w-4 text-purple-600 mt-0.5 flex-shrink-0" />
                    <span><strong>"QSFP = Quad = 4x"</strong> - QSFP transceivers always have 4 channels</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Lightbulb className="h-4 w-4 text-purple-600 mt-0.5 flex-shrink-0" />
                    <span><strong>"SR = Short, LR = Long, ER = Extended"</strong> - Easy distance recall</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Lightbulb className="h-4 w-4 text-purple-600 mt-0.5 flex-shrink-0" />
                    <span><strong>"850 = Multimode, 1310/1550 = Single-mode"</strong> - Wavelength to fiber type</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Lightbulb className="h-4 w-4 text-purple-600 mt-0.5 flex-shrink-0" />
                    <span><strong>"BiDi = Bi-directional = 1 fiber, 2 wavelengths"</strong> - Cost savings</span>
                  </li>
                </ul>
              </div>
            </CardContent>
